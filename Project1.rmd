...
We expect that the distribution of the means will be normal, centered around the theoretical mean. Plotting with the relevant empirical density overlaying the histogram, it is apparent that the distribution is approximately normal and centered around the mean.  The true normal distribution centered at the given mean is shown as an orange line, while the light pink overlay is the empirical density.
...
The code below is used to compute the means for each row of 40 samples, and prepare the plot demonstrating asymptotic convergence of the mean.
...
```{r displayCompare_Mean,ref.label='mean_distribution',eval=FALSE}
